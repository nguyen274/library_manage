$(document).ready(function(){
	...
	$('.btnCapNhatThongTin').on("click", function(event) {
		...
		var href = "http://localhost:8080/library_management/api/profile/"+userId;
		$.get(href, function(user, status) {
			populate('.formCapNhat', user);
		});
	...
	function ajaxPostChangePass() {
	    	// PREPATEE DATA
	    	 ...
	    	 $.ajax({
	 			...
	 			url : "http://localhost:8080/library_management/api/profile/doiMatKhau",
	 			data : JSON.stringify(data),
	 			success : function(response) {
	 				if(response.status == "success"){
	 					$('#doiMKModal').modal('hide');
	 					alert("Đổi mật khẩu thành công. Bạn phải đăng nhập lại để xác nhận");
	 					location.href = "http://localhost:8080/library_management/logout";
	 				} else {
	 			    	$('input').next().remove();
